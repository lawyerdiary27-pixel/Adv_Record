...
  authDomain: "[YOUR_PROJECT_ID.firebaseapp.com](http://your_project_id.firebaseapp.com/)",

  projectId: "YOUR_PROJECT_ID",

  storageBucket: "[YOUR_PROJECT_ID.firebasestorage.app](http://your_project_id.firebasestorage.app/)",
...
import { initializeApp } from "[https://www.gstatic.com/firebasejs/12.2.1/firebase-app.js](https://www.gstatic.com/firebasejs/12.2.1/firebase-app.js)";
...
} from "[https://www.gstatic.com/firebasejs/12.2.1/firebase-auth.js](https://www.gstatic.com/firebasejs/12.2.1/firebase-auth.js)";
...
} from "[https://www.gstatic.com/firebasejs/12.2.1/firebase-firestore.js](https://www.gstatic.com/firebasejs/12.2.1/firebase-firestore.js)";

const cfg=window\.MACT_FIREBASE_CONFIG||{};

const configured=cfg.apiKey && !String(cfg.apiKey).startsWith("PASTE\_") &&

                 cfg.projectId && !String(cfg.projectId).startsWith("YOUR\_");
...
const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbzizLrVzhfeimuDVLxIF2m3uapX8Nj9KaANcIFCOKkOEXFDIlIk5uBeILTA1ewIJAFv/exec";
...
     if(!targetSec.fields.some(f=>f[0]===[cf.id](http://cf.id/))){

      targetSec.fields.push([[cf.id](http://cf.id/), cf.label, cf.type, cf.options]);
...
  const fieldId = "custom\_" + Date.now() + "\_" + Math.random().toString(36).slice(2, 6);
...
    data.judgmentPdfName = [file.name](http://file.name/);
...
  if(pdf){ openPdf([pdf.dataset.id](http://pdf.dataset.id/)); }
...
  const id = [e.target.dataset.id](http://e.target.dataset.id/);
...
  r.judgmentPdfName = [file.name](http://file.name/);
...
 return DEFAULT_HEARING_PRINT_COLUMNS.map(x=>[x.id](http://x.id/));
...
 box.innerHTML=DEFAULT_HEARING_PRINT_COLUMNS.map((f,i)=>\`\<div class="hearing-print-row" draggable="true" data-hearing-print-id="${[f.id](http://f.id/)}">

  \<span class="drag">⠿\</span>\<input type="checkbox" class="hearing-print-check" ${selected.has([f.id](http://f.id/))?"checked":""}>
...
 const defs=Object.fromEntries(DEFAULT_HEARING_PRINT_COLUMNS.map(x=>[[x.id](http://x.id/),x]));
...
if($("hearingPrintResetBtn")) $("hearingPrintResetBtn").onclick=()=>{saveHearingPrintColumns(DEFAULT_HEARING_PRINT_COLUMNS.map(x=>[x.id](http://x.id/)));renderHearingPrintSettings();};
...
  const cb="mactCloudCb\_"+Date.now()+"\_"+Math.random().toString(36).slice(2,8);
...
   if([f.to](http://f.to/) && String(r.accidentDate||"")>[f.to](http://f.to/)) return false;
...
 $("advancedPagination")?.addEventListener("click",e=>{const b=e.target.closest("[data-page]");if(!b)return;const list=advancedList(),pages=Math.max(1,Math.ceil(list.length/advPageSize));const v=[b.dataset.page](http://b.dataset.page/);if(v==="prev")advPage=Math.max(1,advPage-1);else if(v==="next")advPage=Math.min(pages,advPage+1);else advPage=Number(v);renderTable()});
...
  const el=document.createElement("div");[el.id](http://el.id/)="v7MiniStats";el.className="v7-quick-stat";
...
  $("v7CommandPalette")?.addEventListener("click",e=>{if([e.target.id](http://e.target.id/)==="v7CommandPalette")closePalette()});
...
      const {doc,runTransaction,serverTimestamp}=window\.MACTFirebase.\_\_api;
...
      import("[https://www.gstatic.com/firebasejs/12.2.1/firebase-firestore.js](https://www.gstatic.com/firebasejs/12.2.1/firebase-firestore.js)").then(api=>{

        window\.MACTFirebase.\_\_api=api;
...
  document.querySelectorAll(".m11-check").forEach(c=>c.addEventListener("change",()=>{if(c.checked)selected.add(String([c.dataset.id](http://c.dataset.id/)));else selected.delete(String([c.dataset.id](http://c.dataset.id/)));updateHint()}));
...
  const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="MACT_V11_Emergency_Backup\_"+new Date().toISOString().slice(0,10)+".json";a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);
...
  $("mactV11Overlay").addEventListener("click",e=>{if([e.target.id](http://e.target.id/)==="mactV11Overlay")close()});
...
function renderDocs(){const ds=docs(),rs=records();$("m12DocsOut").innerHTML=ds.length?\`\<div style="overflow\:auto">\<table class="m12-table">\<thead>\<tr>\<th>Case\</th>\<th>Type\</th>\<th>Name\</th>\<th>Added\</th>\<th>File\</th>\</tr>\</thead>\<tbody>${ds.map(d=>{let r=rs.find(x=>String(x.recordId)===String(d.caseId));return \`\<tr>\<td>${esc(r?label(r):"Unknown")}\</td>\<td>${esc(d.type)}\</td>\<td>${esc([d.name](http://d.name/))}\</td>\<td>${esc(new Date([d.at](http://d.at/)).toLocaleString())}\</td>\<td>${d.fileName?esc(d.fileName):"Metadata only"}\</td>\</tr>\`}).join("")}\</tbody>\</table>\</div>\`:'\<div class="m12-muted">No documents registered yet.\</div>'}

function addDoc(){const id=$("m12DocCase").value;if(!id){toast("Select a case.");return}const f=$("m12DocFile").files[0],d=docs();d.unshift({id\:crypto.randomUUID?crypto.randomUUID()\:Date.now()+"",caseId\:id,type:$("m12DocType").value,name:$("m12DocName").value||(f&&[f.name](http://f.name/))||"Untitled document",fileName\:f?[f.name](http://f.name/):"",size\:f?f.size:0,at\:new Date().toISOString()});saveDocs(d);audit("Document registered",$("m12DocName").value||"Untitled");$("m12DocName").value="";$("m12DocFile").value="";renderDocs();renderStats();toast("Document registered. File content is not uploaded by this V12 module.");}
